...
func main() {
	...
	fn := func(idx int, p, u string) {
		defer wg.Done()
		comm_LOGD("download: %03d", idx)
		//comm_LOGD("%s", p)
		//time.Sleep(1000000)
		if e := DownloadThread(u, p, nil, 1); e != nil {
			os.Remove(p)
			comm_LOGD("fail: down: %s %v", p, e)
			comm_FwriteString("fail_"+p+".log", fmt.Sprintf("fail: download thread: %v", e))
	} }
	...
	for i, v := range p {
		wg.Add(1)
		go fn(i, v.Name, v.Uri)
		if 0==(i+1)%limit { wg.Wait() }
	}
	if 0!=size%limit { wg.Wait() }
